...
export default async req=>{
 ...
 try{
  const {action,orderId}=await req.json();if(!['list','proof','confirm','delete'].includes(action)||((action==='proof'||action==='confirm'||action==='delete')&&!orderId))return json(400,{error:'Permintaan tidak valid.'});
  ...
  if(action==='delete'){
   if(order.proof_path)await fetch(`${url}/storage/v1/object/payment-proofs/${order.proof_path}`,{method:'DELETE',headers});
   const removed=await fetch(`${url}/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}`,{method:'DELETE',headers});if(!removed.ok)throw new Error('Gagal menghapus transaksi.');return json(200,{deleted:true});
  }
  if(order.status!=='paid'){
   const updated=await fetch(`${url}/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}`,{method:'PATCH',headers:{...headers,'content-type':'application/json',prefer:'return=representation'},body:JSON.stringify({status:'paid',paid_at:new Date().toISOString()})});if(!updated.ok)throw new Error('Gagal mengonfirmasi pembayaran.');
   void sendPush({recipientType:'buyer',orderId:order.id,title:'Pembayaran dikonfirmasi',body:'Produk Banda Pustaka Anda sudah siap diunduh.',url:`/#pesanan=${order.access_token}`}).catch(error=>console.error('Buyer push error:',error.message));
  }
