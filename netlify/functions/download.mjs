const response=(status,body,headers={})=>({statusCode:status,headers,body});
export default async req=>{
 try{const token=req.queryStringParameters?.token;if(!token)return response(400,'Token diperlukan.');const url=process.env.SUPABASE_URL,key=process.env.SUPABASE_SERVICE_ROLE_KEY;const headers={apikey:key,authorization:`Bearer ${key}`};
